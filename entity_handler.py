...
def clean_dna(generation):
    ...
    for entities in generation:
        ...
        #Beggining of Score Cleaning Block
            ...
            #If the score is legit then score_keeper var should be set to equal the current actions score count
            if score_keeper == (dna.getScoreCount() - previous_starting_score) or score_keeper < (dna.getScoreCount() - previous_starting_score):
                score_keeper = (dna.getScoreCount() - previous_starting_score)

                if previous_starting_score!= 0:
                    dna.setScoreCount(score_keeper)

            #If the score count is not legit than the current actions score count should be set to the ring keepers value
            elif score_keeper > (dna.getScoreCount()- previous_starting_score):
                dna.setScoreCount(score_keeper)
        ...
        #Beggining of Ring Cleaning Block
            ...
            #Countinues until first legit zero appears in a action in the action list
            ...
            #If the ring count is legit then ring_keeper var should be set to equal the current actions score count
            elif ring_keeper == dna.getRingCount() or ring_keeper < dna.getRingCount() or dna.getRingCount() == 0:
                ring_keeper = dna.getRingCount()

            #If the ring count is not legit than the current action ring count should be set to the ring keepers value
...
#Function handles the postive evaluation of the dna sequence of each enitiy in the generation gen_list
#Passes:
...
def pos_dna_eval(generation):
    ...
    for ent in pos_gen:
        ...
        while ent.action_list[i].getLivesCount() >= curr_lives:
            ...
            #Positive evaluation for the current action and previous action will occur if the ring or score count has increased
            if ring_keeper < ent.action_list[i].getRingCount() or score_keeper < ent.action_list[i].getScoreCount():

                #If the increases occured within five seconds of previous increase
                #Then the positive reward for the current action an previous action will increase to 0.100
                if delay_keeper <= 5:
                    ent.setActionList(mutation_adjuster(ent.getActionList(), i, 0.100, 10, 'dec'))
                #Else the reward for the current and previous action will be the standarded 0.05
                else:
...
#Function handles the negavtive evaluation of the dna sequence of each enitiy in the generation gen_list
#Passes:
#        Generation: List of entities that belong to the same Generation
#
#Returns:
...
def neg_dna_eval(generation):
    ...
    for ent in generation:
        ...
        while ent.action_list[i].getLivesCount() >= lives_keeper:
            ...
            if sdelay >= 30 and ent.action_list[i].getScoreCount() == score_keeper:
...
#           index:       The place where the modification to the mutatin values will begin in the action_list
#           amount:      The amount the mutation value will increase or decrease
#           delay:       How far back the list will iterator before it reaches the end of its mutation range
